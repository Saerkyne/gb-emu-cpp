...
// This file contains the most common routines for the Game
// Boy emulator.
...
#define cpu_routine_add_hl_16(reg16)                       \
  {                                                        \
    ...
    bool hc = ((cpu_registers.hl & 0x0FFF) +               \
               (reg16 & 0x0FFF)) > 0x0FFF;                 \
...
#define cpu_routine_add_a_8(reg8)                          \
  {                                                        \
    ...
    SET_FLAG_HALF_CARRY(                                   \
      ((temp & 0xF) + ((uint32_t) reg8 & 0xF)) > 0xF);     \
...
#define cpu_routine_adc_a_8(reg8)                          \
  {                                                        \
    ...
    bool hc = (((cpu_registers.a & 0xF) + (reg8 & 0xF)) +  \
               carry) > 0xF;                               \
    SET_FLAG_HALF_CARRY(hc);                               \
...
#define cpu_routine_sub_a_8(reg8)                          \
  {                                                        \
    SET_FLAG_SUBTRACT(1);                                  \
    SET_FLAG_HALF_CARRY((cpu_registers.a & 0xF) <          \
                        (reg8 & 0xF));                     \
    SET_FLAG_CARRY((uint32_t) cpu_registers.a <            \
                   (uint32_t) reg8);                       \
    cpu_registers.a -= reg8;                               \
    SET_FLAG_ZERO(cpu_registers.a == 0);                   \
    core_advance_cpu_clocks(4);                            \
...
#define cpu_routine_sbc_a_8(reg8)                          \
  {                                                        \
    uint16_t temp =                                        \
      cpu_registers.a - (reg8 + GET_FLAG_CARRY);           \
    SET_FLAG_SUBTRACT(1);                                  \
    SET_FLAG_CARRY((temp & ~0xFF) ? 1 : 0);                \
    SET_FLAG_ZERO((temp & 0xFF) ? 0 : 1);                  \
    SET_FLAG_HALF_CARRY(                                   \
      ((cpu_registers.a ^ reg8 ^ temp) & 0x10) != 0);      \
    cpu_registers.a = temp;                                \
    core_advance_cpu_clocks(4);                            \
  }
...
#define cpu_routine_rst_nnnn(addr)                         \
  {                                                        \
    ...
    const uint8_t pc_high =                                \
      (cpu_registers.pc & 0xFF00) >> 8;                    \
    core_advance_cpu_clocks(4);                            \
...
// TODO: check if the second memory bus write should be the
// low byte. Original implementation used high byte.
...
#define cpu_routine_call_conditional_nnnn(cond)            \
  {                                                        \
    ...
    if (cond) {                                            \
      ...
      core_advance_cpu_clocks(4);                          \
      temp |=                                              \
        ((uint32_t) memory_bus_read(cpu_registers.pc++))   \
        << 8;                                              \
      core_advance_cpu_clocks(4);                          \
      cpu_registers.sp--;                                  \
      cpu_registers.sp &= 0xFFFF;                          \
...
#define cpu_routine_ret_conditional(cond)                  \
  {                                                        \
    ...
    if (cond) {                                            \
      ...
      core_advance_cpu_clocks(4);                          \
      temp |=                                              \
        ((uint32_t) memory_bus_read(cpu_registers.sp++))   \
        << 8;                                              \
      cpu_registers.sp &= 0xFFFF;                          \
      core_advance_cpu_clocks(4);                          \
      cpu_registers.pc = temp;                             \
...
#define cpu_routine_jr_conditional_n(cond)                 \
  {                                                        \
    ...
    if (cond) {                                            \
      ...
      cpu_registers.pc =                                   \
        (cpu_registers.pc + (int8_t) temp) & 0xFFFF;       \
      core_advance_cpu_clocks(4);                          \
    } else {                                               \
      cpu_registers.pc++;                                  \
      core_advance_cpu_clocks(4);                          \
